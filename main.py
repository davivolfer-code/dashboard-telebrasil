...
# --- ROTA DE UPLOAD (ATUALIZADA COM MESCLAGEM) ---
        ...
        if os.path.exists(JSON_PATH):
            with open(JSON_PATH, 'r', encoding='utf-8') as f:
                try:
                    ...
                    # Criamos um dicionário rápido usando CNPJ como chave
                    cache_existente = {str(c['cnpj']): c for c in dados_existentes if 'cnpj' in c}
                    
                    for novo in dados_novos:
                        cnpj_key = str(novo['cnpj'])
                        if cnpj_key in cache_existente:
                            # Se o cliente já existia, preservamos as edições manuais
                            ex = cache_existente[cnpj_key]
                    ...
                    pass # Se o arquivo estiver corrompido, segue com os novos
